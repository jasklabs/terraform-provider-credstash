...
release:
	...
	docker run --rm -v `pwd`:/usr/local/go/src/github.com/sspinc/terraform-provider-credstash -w /usr/local/go/src/github.com/sspinc/terraform-provider-credstash golang:alpine go build -v -o build/linux_amd64_musl/terraform-provider-credstash_$(VERSION)
